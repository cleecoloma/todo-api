...
const userModel = (sequelize, DataTypes) => {
  const userTable = sequelize.define('Users', {
    ...
    role: {
      type: DataTypes.ENUM('admin', 'user', 'editor', 'writer'),
      allowNull: false,
    },
    ...
    token: {
      ...
      get() {
        // this function runs whenever I refer to this property on the record
        return jwt.sign(
          { username: this.username, capabilities: this.capabilities },
          SECRET
        ); // function for creating our claim.
      },
    },
    capabilities: {
      type: DataTypes.VIRTUAL,
  ...
  userTable.authenticateBasic = async function (username, password) {
    let userRecord = await this.findOne({ where: { username } }); // values are pulled from DB
    let valid = await bcrypt.compare(password, userRecord.password);
    console.log("HERES THE USERNAME AND PASSWORD ", username, password)
    if (valid) {
      return userRecord;
    } else {
